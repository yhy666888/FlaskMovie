...
class Comment(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'))  # 所属用户，在user表中创建关联
    add_time = db.Column(db.DateTime, index=True, default=datetime.datetime.now)  # 添加时间
...
class Admin(db.Model):
    ...
    operatelogs = db.relationship('OperateLog', backref='operatelog')  # 管理员日志外键关系关联
...
# if __name__ == '__main__':
    ...
    # admin = Admin(
    #     name='admin',
    #     pwd=generate_password_hash('password'),  # 加密密码
    #     is_super=0,
    #     role_id=1,
    # )
